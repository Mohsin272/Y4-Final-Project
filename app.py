...
@app.route("/edamam")
def edamam():
    return render_template("edamam.html", title="Edamam", heading="Enter the following for a Personalised Recipe")
...
def process():
    ...
    if health =="" or " ":
        ...
        result = requests.get(f'https://api.edamam.com/search?q={ingredients}&health={health}&app_id={app_id}&app_key={app_key}')
    else:
